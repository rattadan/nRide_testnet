...
update_app_toml() {
  # Update API address
  current_api_address=$(grep -E '^\s*address\s*=' ~/.gaia/config/app.toml | grep -m 1 "tcp" | cut -d '=' -f 2 | tr -d ' ')
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    ...
    echo
    sed -i "/^\[api\]/,/^\[/ { /^\s*address\s*=/ s|address = \".*\"|address = \"$new_api_address\"| }" ~/.gaia/config/app.toml
  fi

  # Update gRPC address
  current_grpc_address=$(sed -n '/^\[grpc\]/,/\[/{ /address =/p }' ~/.gaia/config/app.toml | cut -d '=' -f 2 | tr -d ' ')
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    ...
    echo
    sed -i "/^\[grpc\]/,/^\[/ { /^\s*address\s*=/ s|address = \".*\"|address = \"$new_grpc_address\"| }" ~/.gaia/config/app.toml
  fi
}

# Function to update the chain ID in client.toml
update_client_toml() {
  current_chain_id=$(grep -E '^\s*chain-id\s*=' ~/.gaia/config/client.toml | cut -d '=' -f 2 | tr -d ' ')
  read -p "Current chain ID: $current_chain_id - Do you want to change it? [y/N] " REPLY
  echo
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    read -p "Enter new chain ID: " new_chain_id
    echo
    sed -i "s|chain-id = \".*\"|chain-id = \"$new_chain_id\"|" ~/.gaia/config/client.toml
  fi
}

# Function to update proxy_app and other addresses in config.toml
update_config_toml() {
  current_proxy_app=$(grep -E '^\s*proxy_app\s*=' ~/.gaia/config/config.toml | cut -d '=' -f 2 | tr -d ' ')
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    ...
    echo
    sed -i "s|proxy_app = \".*\"|proxy_app = \"$new_proxy_app\"|" ~/.gaia/config/config.toml
  fi

  current_rpc_address=$(grep -E '^\s*caddr\s*=' ~/.gaia/config/config.toml | cut -d '=' -f 2 | tr -d ' ')
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    ...
    echo
    sed -i "s|caddr = \".*\"|caddr = \"$new_rpc_address\"|" ~/.gaia/config/config.toml
  fi

  current_laddr=$(grep -E '^\s*laddr\s*=' ~/.gaia/config/config.toml | cut -d '=' -f 2 | tr -d ' ')
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    ...
    echo
    sed -i "s|laddr = \".*\"|laddr = \"$new_laddr\"|" ~/.gaia/config/config.toml
  fi
}
