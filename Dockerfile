...
# then log output of file /code/bin/nrided
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/nrided \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/nrided | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/nrided /usr/bin/nrided
