...
HTTPS_GIT := github.com/rollchains/nridetestnet.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nridetestnet \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=nrided \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/nrided ./cmd/nrided
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/nrided.exe ./cmd/nrided
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/nrided
...
draw-deps:
	...
	@goviz -i ./cmd/nrided -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c nridetestnet --local -f chains.yaml
endif
...
set-testnet-configs:
	nrided config set client chain-id localchain-1
	nrided config set client keyring-backend test
	nrided config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | nrided keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | nrided keys add acc1 --recover`
